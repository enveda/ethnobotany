...
def clade_angles(tree, arc, start):
    ...
    count = len(tree.get_leaves())
    leaves_distances = (arc / (count - 1))

    angles = {
        tip: start + (leaves_distances * i)
        for i, tip in enumerate(reversed(tree.get_leaves()))
    }

    if tree.root.clades:
        internal_clades_angles(tree.get_tree_root(), angles)
...
def draw_internal_wedge(clade, angles, ax, depth):
    min_angle = min(angles[child]
                    for child in clade.get_leaves())

    max_angle = max(angles[child]
                    for child in clade.get_leaves())

    nnodes = len(clade.get_leaves())
